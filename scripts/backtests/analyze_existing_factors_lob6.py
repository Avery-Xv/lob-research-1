#!/usr/bin/env python3
"""LOB-six style exposure and neutralized backtests for existing factors."""
...
def load_common(
    *,
    ...
    style_select = ", ".join(f"{column}::DOUBLE AS {column}" for column in LOB6_COLS)
    style_current = ", ".join(f"s.{column}::DOUBLE" for column in LOB6_COLS)
    style_previous = ", ".join(f"s.previous_{column}::DOUBLE" for column in LOB6_COLS)
    previous_style_select = ", ".join(
        f"lag({column}) OVER (PARTITION BY symbol ORDER BY date) AS previous_{column}"
        for column in LOB6_COLS
    )
    connection.execute(
        f"""
...
def exposure_by_date(
    ...
    for key, rows in sorted(grouped.items()):
        ...
        for index, style_name in enumerate(LOB6_COLS):
            ...
            output.append(
                {
                    ...
                    "lob6_rank_ic": pearson(neutral_ranks, return_ranks),
                    "raw_d10_d1": spread([returns[index] for index in raw_order]),
                    "lob6_d10_d1": spread([returns[index] for index in neutral_order]),
                }
            )
    return output
...
def summarize_performance(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for key, observations in sorted(grouped.items()):
        ...
        for metric in ("raw_rank_ic", "lob6_rank_ic", "raw_d10_d1", "lob6_d10_d1"):
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "results/lob6/existing_factors_202601"))
