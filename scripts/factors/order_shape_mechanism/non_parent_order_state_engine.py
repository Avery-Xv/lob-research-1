"""Leakage-safe fixed-grid non-parent order-state engine."""
...
FACTOR_VERSION = "non_parent_order_state_fixed_1030_local_history_v2_20260808"
HISTORY_SNAPSHOT_VERSION = 2
SIDES = ("B", "S")
TARGET_SIGNAL_SECONDS = 10.5 * 3600
MODEL_ORDER_CUTOFF_SECONDS = 10 * 3600 + 40 * 60


def profile_grid_seconds() -> tuple[float, ...]:
    """Historical state samples needed by the fixed 10:30 specification."""
    return tuple(10 * 3600 + minute * 60 for minute in (0, 10, 20, 30))


PROFILE_GRID_SECONDS = profile_grid_seconds()


@dataclass(frozen=True)
class NonParentOrderStateConfig:
...
@dataclass
class NonParentOrderStateQuality:
    total_events: int = 0
    valid_trades: int = 0
...
class NonParentOrderStateEngine:
    """Process all months for one symbol in chronological event order."""

    def __init__(self, symbol: str, config: NonParentOrderStateConfig | None = None) -> None:
        self.symbol = symbol
        self.config = config or NonParentOrderStateConfig()
        ...
        self.quality = NonParentOrderStateQuality()
    ...
    def export_history_snapshot(self) -> dict[str, object]:
        """Finalize history and serialize only state needed by later target months."""
        self._finish_day()
        reservoirs = []
        for (bucket, side), reservoir in sorted(self.profile_reservoirs.items()):
            reservoirs.append({
                "bucket": bucket,
                "side": side,
                "capacity": reservoir.capacity,
                "seen": reservoir.seen,
                "values": list(reservoir.values),
                "random_state": repr(reservoir._random.getstate()),
            })
        return {
            "kind": "non_parent_fill_history_snapshot",
            "schema_version": HISTORY_SNAPSHOT_VERSION,
            "factor_version": FACTOR_VERSION,
            "symbol": self.symbol,
            "config": {
                "observation_seconds": self.config.observation_seconds,
                "intensity_reservoir_size": self.config.intensity_reservoir_size,
                "minimum_fill_history": self.config.minimum_fill_history,
                "near_bps": self.config.near_bps,
            },
            "fill_cells": [
                {"side": side, "state": state, "distance": distance,
                 "orders": values[0], "filled": values[1]}
                for (side, state, distance), values in sorted(self.fill_cells.items())
            ],
            "fill_globals": [
                {"side": side, "distance": distance,
                 "orders": values[0], "filled": values[1]}
                for (side, distance), values in sorted(self.fill_globals.items())
            ],
            "profile_reservoirs": reservoirs,
        }

    def load_history_snapshot(self, snapshot: dict[str, object]) -> None:
        """Restore a point-in-time history state before processing a target month."""
        if self.current_date is not None or self.signal_rows or self.quality_rows:
            raise ValueError("history snapshot must be loaded into a fresh engine")
        if snapshot.get("kind") != "non_parent_fill_history_snapshot":
            raise ValueError("unexpected history snapshot kind")
        if snapshot.get("schema_version") != HISTORY_SNAPSHOT_VERSION:
            raise ValueError("unsupported history snapshot schema")
        if snapshot.get("factor_version") != FACTOR_VERSION:
            raise ValueError("history snapshot factor version mismatch")
        if snapshot.get("symbol") != self.symbol:
            raise ValueError("history snapshot symbol mismatch")
        expected = {
            "observation_seconds": self.config.observation_seconds,
            "intensity_reservoir_size": self.config.intensity_reservoir_size,
            "minimum_fill_history": self.config.minimum_fill_history,
        ...
        if snapshot.get("config") != expected:
            raise ValueError("history snapshot config mismatch")
        self.fill_cells.clear()
        for row in snapshot.get("fill_cells", []):
            key = (str(row["side"]), str(row["state"]), str(row["distance"]))
            self.fill_cells[key] = [int(row["orders"]), int(row["filled"])]
        self.fill_globals.clear()
        for row in snapshot.get("fill_globals", []):
            key = (str(row["side"]), str(row["distance"]))
            self.fill_globals[key] = [int(row["orders"]), int(row["filled"])]
        self.profile_reservoirs.clear()
        for row in snapshot.get("profile_reservoirs", []):
            reservoir = Reservoir(int(row["capacity"]), 0)
            reservoir.seen = int(row["seen"])
            reservoir.values = [float(value) for value in row["values"]]
            reservoir._random.setstate(ast.literal_eval(str(row["random_state"])))
            self.profile_reservoirs[(str(row["bucket"]), str(row["side"]))] = reservoir
    ...
    def _schedule_grids_before(self, seconds: float) -> None:
        while self.next_grid < len(PROFILE_GRID_SECONDS):
            grid = PROFILE_GRID_SECONDS[self.next_grid]
            if self.current_session != "AM" or grid > seconds:
                break
    ...
    def _sample_grid(self, grid: float) -> None:
        ...
        if (
            grid != TARGET_SIGNAL_SECONDS
            or self.current_date is None
            or self.current_date // 100 != self.target_month
        ):
            return
    ...
    def _update_order_model(self, event: Event, seconds: float, valid_trade: bool) -> None:
        if (
            event.action == "ORDER_ADD"
            and seconds < MODEL_ORDER_CUTOFF_SECONDS
            and event.side in SIDES
            and event.price is not None
            and (event.volume or 0) > 0
            and self.previous_book is not None
        ):
    ...
    def _start_day(self, date: int) -> None:
        ...
        buckets = {self._seconds_bucket(seconds) for seconds in PROFILE_GRID_SECONDS}
        self.day_thresholds = {
    ...
    def _start_session(self, session: str) -> None:
        ...
        self.chain = None
        self.pending.clear()
        self.next_grid = 0 if session == "AM" else len(PROFILE_GRID_SECONDS)
