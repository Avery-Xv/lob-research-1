#!/usr/bin/env python3
"""Run symmetric M1 chain pre/post comparisons with one minimal V4 read."""
...
SELECT date::INTEGER, time::BIGINT, row_id::BIGINT, source_action,
       ...
       source_sell_order_id::BIGINT, source_side, NULL::BIGINT,
...
QUALITY_FIELDS = [
    "symbol",
    "date",
    *M1PrePostQuality.__dataclass_fields__,
    "factor_version",
]
...
def process_symbol(
    ...
    engine = M1PrePostEngine(symbol, config)
    try:
...
def build_manifest(
    ...
    body = {
        ...
        "window_rule": (
            "equal pre/post windows around contiguous same-side same-active-order-id "
            "trade chains; chain volume excluded; no session crossing"
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run target-only symmetric M1 pre/post flow experiment.")
...
def main() -> int:
    ...
    inputs, metadata = load_inputs(args.file_list, args.universe_metadata, [], target_month)
        ...
        with ProcessPoolExecutor(max_workers=args.workers, mp_context=context) as executor:
            ...
            for completed, future in enumerate(as_completed(futures), start=1):
                ...
                print(
                    ...
                    f"symbols={completed_symbols}/{len(symbols)} stat_rows={stat_rows} "
                    f"quality_rows={quality_rows}",
                    flush=True,
                )
    return 0
