...
| 非母单订单状态候选 | 成交机会、盘口与成交流背离、撤单强度有历史机制线索，但尚无正式全市场结果 | **当前优先筛选** | 固定 10:30 全市场缓存完成后，分九类股票检查未来订单流、盘口和波动 | F014 |
...
1. 完成固定 10:30 的全市场订单状态缓存，并优先筛选五个非母单候选（登记 F014）；
...
| 超出历史预期的成交机会 | R014；NP01 | 共享缓存计算中 | F014、P002 | 历史小幅新增信息 |
| 盘口总体是否容易成交 | R015；NP02 | 共享缓存计算中 | F014、P002 | 无正式结果 |
| 主动流和成交机会相互确认还是冲突 | R016；NP03 | 共享缓存计算中 | F014、P002 | 无正式结果 |
| 盘口方向与实际成交流向背离 | R017；NP04 | 共享缓存计算中 | F014、P002 | 历史机制较强 |
| 两侧撤单强度与流动性风险 | R018；NP05 | 共享缓存计算中 | F014、P002 | 旧定向版本较强 |
...
本章对应因子登记 F014。这五项不研究隐藏母单，也不需要重新扫描原始逐笔数据；它们读取固定 10:30、记录了历史模型版本的共享缓存。当前全市场缓存目标为 5,160 只股票×20日，完成后最多 103,200 行信号。
