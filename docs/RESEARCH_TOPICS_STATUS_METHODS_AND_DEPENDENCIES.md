...
```mermaid
flowchart TD
    Q[Q001-Q008<br/>事件语义、股票池、时间边界、串并行等质量门]
    P1[P001 共享 LOB 事件与生命周期原语]
    P2[P002 固定10:30非母单订单状态缓存]

    Q --> P1
    Q --> P2

    F1[F001 主动成交价格冲击] --> R2[R002 AG01跨月复现]
    F6[F006 D04/D05] --> R3[R003 主动大单历史意外]
    F7[F007 D06定义] --> R4[R004 价格反应不足]

    P1 --> F8[F008 D07]
    F8 --> R5[R005 冲击永久性与恢复]
    F6 --> R6[R006 主动流-被动承接四状态]
    F8 --> R6
    F9[F009 D08状态] --> R6
    P1 --> F10[F010 D09]
    F10 --> R7[R007 极端报价修复]
    P1 --> F11[F011 D10]
    F11 --> R8[R008 流动性条件化冲击]
    P1 --> F12[F012 D11]
    F12 --> R9[R009 异常价格冲击]

    P1 --> ELOB[R010-R013<br/>Event-LOB四项最小实证]

    P2 --> F13[F013 M1-M6基线]
    F13 --> R1[R001 六机制基线]
    P2 --> F14[F014 NP01-NP05]
    F13 --> F14
    F14 --> NP[R014-R018<br/>五项非母单候选]

    P1 --> F15[F015 NPE01-NPE05]
    ELOB --> F15
    F15 --> R19[R019 非母单事件深化]

    P1 --> F16[F016 DR01-DR05]
    F16 --> R20[R020 近端大单遮挡]

    P1 --> F17[F017 MSI01-MSI10]
    F13 --> F17
    F17 --> R21[R021 集中度与碎片化]
    R21 -->|P1通过| R22[R022 节奏与结束风险]
    R21 -->|P1通过| R23[R023 冲击、恢复与韧性]
    R21 -->|P1通过| R24[R024 片段可识别性]
    R22 --> R25[R025 综合增量与收益]
    R23 --> R25
    R24 --> R25
    F13 --> R25

    classDef running fill:#dbeafe,stroke:#2563eb,color:#111827;
    ...
    class P2,F14,NP running;
    class R1,F13 done;
    class R5,R7,R8,F8,F10,F11 partial;
    class R4,R6,R9,R19,R20,R21,R22,R23,R24,R25 gated;
```
