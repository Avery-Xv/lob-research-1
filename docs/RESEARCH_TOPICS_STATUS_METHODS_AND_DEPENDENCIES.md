...
本文以因子候选及其研究决策为主线，并使用 `research/experiments.json` 中正式登记的 R001–R025 作为证据索引。它说明：

1. 当前有哪些因子族、基准和候选信号；
2. 每个候选已经获得什么机制或收益证据；
3. 相对 M1、活动度等已有解释是否仍有增量；
4. 当前决定是继续、修复、等待上游还是停止；
5. 下一步最有信息价值的检验是什么；
6. 各项结论由哪些 R 实验、P 数据产品和 Q 质量门支持。
...
本文的主对象是 **F 因子或候选定义**。R 是为了判断 F 是否值得继续而执行的研究实验；P 是共享中间数据；Q 是质量门。一个 F 可能需要多个 R 才能完成机制、增量、跨月和收益检验，一个 R 也可能为多个后续候选提供公共证据。因此不能把 R001–R025 直接理解为 25 个待上线因子。
...
```mermaid
flowchart LR
    H[经济机制假说] --> F[可在信号时点计算的 F 候选]
    F --> D[未来订单流、冲击、恢复、波动等直接目标]
    D --> I{控制 M1 与活动度后仍有增量?}
    I -->|否| S[停止、改定义或降级为条件状态]
    I -->|是| X[跨域、跨月和 placebo]
    X --> A[信号后收益与可成交收益]
    A --> C{成本和样本外仍成立?}
    C -->|否| S
    C -->|是| N[生产候选]
    QP[Q 质量门与 P 共享数据] -.->|提供可信输入，不替代研究结论| F
    QP -.-> D
```

### 2.1 当前因子候选组合

| 因子方向 | F 候选或基准 | 现在知道什么 | 当前决策 | 下一项关键工作 |
|---|---|---|---|---|
| 主动成交价格冲击 | F001 | 旧版单月尾部收益较强，但旧 pre-book 实现不满足当前 V4 语义 | **修复后重算**，不能沿用旧收益结论 | 按 safe-prebook v2 重算沪深，再做跨月和可成交收益 |
| 主动大单流及反应 | F006–F012 | F006 已可计算；F008、F010、F011 只有深圳或部分机制证据；其余定义未冻结 | **优先推进 F006；修复 F008/F010/F011；暂不推进未定义项** | R003 正式计算；补齐上海主动余量和安全 pre-book |
| 非母单订单状态基准 | F013（M1–M6） | M1 是强控制基准；M2/M3 原方向失败；M6 控制 M1 后只剩小增量 | **保留基准，不包装成新 alpha** | 后续候选统一报告控制 M1/活动度前后结果 |
| 非母单订单状态候选 | F014（NP01–NP05） | NP01、NP04、NP05 有历史机制线索，但尚无正式全市场结果 | **当前最高优先级候选组之一** | P002 完成后直接做九域 raw 直接目标和继续/停止表 |
| Event-LOB 公共机制基准 | P001、R010–R013 | 用于建立下一跳、多跳、兑现时间和事件 IRF 基准 | **这是公共证据层，不是待上线因子** | P001 稳定后先完成四项最小实证 |
| 非母单事件深化 | F015（NPE01–NPE05） | 只有研究动机和定义，尚无正式结果 | **等待公共事件基准** | R010–R013 稳定后再实现，避免重复扫描和口径分叉 |
| 近端大单遮挡 | F016（DR01–DR05） | 设计完整，但没有正式机制证据 | **暂缓大规模计算** | 先用 P001 做最小事件机制检验 |
| 母单结构与推断执行片段 | F017（MSI01–MSI10） | 只能从可观测事件推断执行片段，不能等同于真实投资者母单；旧链流大部分可被 M1 和活动度解释 | **只做最小增量门** | 先做 R021；失败则停止 R022–R025 |

### 2.2 如何理解“完成”和“成功”

- **课题完成**只表示某个 R 按预注册方法给出了结果，不表示对应 F 成为有效因子。
- **机制通过**表示候选能预测预设直接目标，并且不是 M1、活动度或盘口水平的简单改名。
- **收益通过**表示信号形成之后的收益方向稳定，不使用未来窗口筛样本或选择阈值。
- **生产候选**还必须通过跨月、九域、样本外、可成交报价、成本、换手和多重检验。
- **停止也是有效产出**。例如 M2/M3 原方向未复现，应记录停止，而不是更换标签后继续寻找显著结果。

### 2.3 当前最短执行路径

当前最有信息价值的顺序是：

1. 完成 P002，并优先筛选 F014 的五个候选；
2. 同时正式计算已经就绪的 F006；
3. 修复 F001、F008、F010、F011 的沪深事件口径后重算；
4. 稳定 P001，完成 R010–R013 公共机制基准；
5. 只让通过直接目标和增量门的候选进入跨月与收益检验；
6. 对母单结构与推断执行片段 F017 先执行 R021，未通过就停止更复杂的片段研究。

后文第 3 节给出统一口径，第 4 节是 R 实验与证据索引，第 5–10 节保存逐项方法和结论，第 12 节记录依赖，第 13 节记录执行顺序。
...
本节登记 R001–R025 的执行状态，服务于证据追溯，不代表 25 个并列的因子候选。判断当前因子组合和研究优先级时，应先看第 2 节；需要了解某项结论由哪个实验产生时，再查本表。

| 实验 | 简称 | 当前状态 | 关键依赖 | 现有证据层级 |
|---|---|---|---|---|
...
## 6. 因子族 A：主动流、价格冲击与盘口恢复（F006–F012）

### 6.1 R003：主动大单流与历史意外增量（D04/D05）
...
### 6.2 R004：主动大单价格反应不足（D06 intraday）
...
## 7. 公共机制基准：Event-LOB 最小实证（不是新因子族）
...
### 7.1 R010：Imbalance × spread 与下一次中价变化
...
### 7.4 R013：Add/Cancel/Trade 事件价格 IRF
...
## 9. 因子族 B2/C：非母单事件深化与大单遮挡（F015–F016）

### 9.1 R019：非母单事件机制深化（NPE01–05）
...
本因子族研究可观测订单与成交是否形成具有增量信息的执行结构。这里的“母单结构”是经济假说，“推断执行片段”是可检验对象；即使规则能够恢复某种片段，也不能据此声称识别了真实投资者母单。

### 10.1 R021：执行链集中度与碎片化增量（MSI-A）
...
### 10.3 R023：边际冲击、穿透恢复与吸收韧性（MSI-C）
